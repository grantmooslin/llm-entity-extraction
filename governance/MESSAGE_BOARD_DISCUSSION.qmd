...
::: {.entry data-date="2026-09-03" class="agent-opencode" data-agent="opencode" data-card="KANBAN-109"}
**2026-09-03 — opencode — KANBAN-109 SHIPPED: interactive asks-questions experiment-flow wizard (branch `hitl-cli-flow`)** Landed `scripts/eval/experiment_flow.py` — a thin HITL wizard over extracted pure functions, orchestrating ONLY existing machinery (`guided_vllm_benchmark.py` + `run_langfuse_docclass_eval.py`, both untouched — the parallel builder owns those copies). ASK: legs `openrouter|modal|both`, keys via `getpass` (never echoed/printed), models `qwen/qwen3-8b` / `Qwen/Qwen3-8B`, prompt default `sorter_v3`, dataset source `braintrust|local|hf` (+ HF knobs, default `Lucius-Morningstar/mailroom-corpus`), sample/seed/max-concurrency, GPU L4 + live hourly rate, spend cap, experiment names — every answer validated, re-prompted on invalid. PREVIEW: exact per-phase commands (cold smoke / warm-up / warm sample / OpenRouter baseline) with env knobs + estimated max spend; explicit y/N before EVERY paid step. RUN: subprocess streaming with per-phase env switching (`OPENROUTER_BASE_URL`/`OPENROUTER_API_KEY` flip, `SERVING_PHASE` cold|warm|unset, `BENCH_GPU`/`BENCH_GPU_HOURLY_USD`, `MODAL_VLLM_*`), spend-cap enforcement (`spend_cap_check`), clean stop between phases. COMPARE: pairs openrouter+modal records from `reports/experiment_log.jsonl` via dataset-fingerprint + prompt-version/sha verification (mismatches REJECTED), prints per-input/output/total-token cost, cold-start amortization, break-even sketch. Scoring goes through the `llm-dojo-scoring` package (63d73d3) ONLY via a runtime subprocess shim (PYTHONPATH prepend `C:/Users/grant/fork-sync/worktrees/cost-comparison-scoring`; never imported, never read) — signatures introspected in the shim and adapted with keyword args in try/except; unavailable values print N/A + reason, never fabricated. Verified end-to-end against the real package (price_for/token_cost_breakdown/estimate_cost/estimate_billed_gpu_cost/amortize_cold_start/effective_tokens_per_sec/compare_api_vs_gpu all adapted, incl. ThroughputMetrics dataclass construction). Pure logic: `validate_config` + per-field validators, `build_plan`/`build_*_command`, `build_plan_env`/`env_for_step`, `verify_pair`/`find_matching_pair`, `redact`/`redacted_env`, `spend_cap_check`, `dojo_call` (stubbed in tests). **Tests: 43 passed** (`tests/test_experiment_flow.py`, network-free, subprocess stubbed) + `test_serving_meta` 14 + `test_hf_docclass_corpus` 8 + docclass/subtype smokes green. Governance: CHANGELOG `[Unreleased]` + card `done` same commit. KANBAN-108 reserved by the parallel incorporation builder (merge-reconciliation note). No Modal deploy/benchmark, no OpenRouter calls, no network in tests, no secrets committed, no push. Card → `done` (archive at next release sweep).
:::
::: {.entry data-date="2026-09-03" class="agent-opencode" data-agent="opencode" data-card="KANBAN-109"}
**2026-09-03 — opencode — KANBAN-109 claimed: interactive asks-questions experiment-flow wizard (HITL) for the OpenRouter vs Modal Qwen3-8B cost comparison** Human directive: build the wizard on branch `hitl-cli-flow` in this worktree, orchestrating EXISTING machinery — `scripts/eval/guided_vllm_benchmark.py` (plus `modal/benchmark_throughput.py` + `modal/vllm_server.py`; read-only bases, driven via subprocess + env knobs `MODAL_APP_NAME`/`MODAL_RUN_ID`/`BENCH_GPU`/`BENCH_GPU_HOURLY_USD`, NOT edited — the parallel builder commits verbatim copies) and `scripts/eval/run_langfuse_docclass_eval.py` (`--dataset-source`, `--hf-*`, `SERVING_PHASE`, `--experiment-log`, `--dry-run`; append-only `reports/experiment_log.jsonl` records). Recon: the dojo scoring package (63d73d3) is importable at runtime only via PYTHONPATH-prepended subprocess — introspected signatures at runtime (allowed); the task's described signatures drift from the actual package (`token_cost_breakdown(prompt_tokens, completion_tokens, model)` not `(prices, ...)`; `estimate_billed_gpu_cost(metrics, ...)` / `compare_api_vs_gpu(..., metrics, ...)` take a `ThroughputMetrics` dataclass; `amortize_cold_start(cold_start_seconds, gpu_hourly_usd, n_runs)`; `effective_tokens_per_sec(tokens_generated, generation_time_sec, cold_start_seconds)`) so the shim introspects + adapts keyword args in try/except, surfacing None/reason on drift. The docclass runner at base 6552289 does NOT yet write a `serving` block (KANBAN-106 wired only the subtype runner), so pairing reads `serving` when present and falls back to `data_source.dataset_fingerprint` + `prompt_versions`; a serving-less api record's provider cannot be confirmed → honest rejection with the reason, never a guess. Plan: new `scripts/eval/experiment_flow.py` + `tests/test_experiment_flow.py` (43 network-free pins), CHANGELOG + board same commit; no paid ops / no network in tests / no secrets / no push. Card → `in_progress`.
:::
